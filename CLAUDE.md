...
| 方法 | 路径                        | 说明                           |
|------|-----------------------------|--------------------------------|
| GET  | `/api/years`                | 返回所有有记录的年份列表（合并 chronology / selected_works / collected_works / manuscript） |
| GET  | `/api/events/{year}`        | 返回某年所有事件（按 literature.id 顺序聚合各文献表；非年谱表以 title+content 作为 event） |
...
- **事件卡片**：固定于英雄区底部，显示「年 月 日」格式日期；同一天按 `literature` 表顺序依次展示各文献内容（正文段首空两个全角空格 `\u3000\u3000`、注释、出处），不同文献之间以虚线分隔
- **底部月份条（month strip）**：横向展示该年所有有数据的月份（按实际 month 字段解析，如 `03`→三月），点击切换月份
- **双指触控板手势（wheel 事件）**：
